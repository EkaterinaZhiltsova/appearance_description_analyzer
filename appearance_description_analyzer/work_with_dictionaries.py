...
def add_to_dict(dict_type, word, word_theme='описание'):
    if dict_type == 0:
        required_thesaurus_file = open(os.path.join(b, 'required_thesaurus_normal_forms.txt'), 'a', encoding='utf-8')
        required_thesaurus_file.write(word + "," + "," + word_theme + '\n')
        required_thesaurus_file.close()
        # Преобразование словаря в нормальную форму
        normal_dict.make_thesaurus_normal_forms(os.path.join(b, 'required_thesaurus_normal_forms.txt'),
                                                os.path.join(b, 'required_thesaurus_normal_forms.txt'))
        return "Слово записано в обязательный словарь"
    elif dict_type == 1:
        possible_thesaurus_file = open(os.path.join(b, 'possible_thesaurus_normal_forms.txt'), 'a', encoding='utf-8')
        possible_thesaurus_file.write(word + "," + "," + word_theme + '\n')
        possible_thesaurus_file.close()
        # Преобразование словаря в нормальную форму
        normal_dict.make_thesaurus_normal_forms(os.path.join(b, 'possible_thesaurus_normal_forms.txt'),
                                                os.path.join(b, 'possible_thesaurus_normal_forms.txt'))
...
def remove_from_dict(dict_type, word):
    ...
    if dict_type == 0:
        text = open(os.path.join(b, 'required_thesaurus_normal_forms.txt'), 'r', encoding='utf-8').read()
        if text.find(normal_word) == -1:
            return "Данного слова нет в обязательном словаре"
        else:
            text = text.replace(normal_word, '')
            required_thesaurus_file = open(os.path.join(b, 'required_thesaurus_normal_forms.txt'), 'w', encoding='utf-8')
        ...
        text = open(os.path.join(b, 'possible_thesaurus_normal_forms.txt'), 'r', encoding='utf-8').read()
        if text.find(normal_word) == -1:
            return "Данного слова нет в необязательном словаре"
        else:
            text = text.replace(normal_word, '')
            possible_thesaurus_file = open(os.path.join(b, 'possible_thesaurus_normal_forms.txt'), 'w', encoding='utf-8')
